...
/***** LLVM Stack map *****/
/**
 * LLVM's Documentation: https://llvm.org/docs/StackMaps.html#stack-map-format
 *
 ...
 */
 ...
 numRecords of the following {
    ...
    uint32_t padding;  // only if required to align to 8 byte
...
#define STACK_MAP_VERSION 3

typedef struct __attribute__((packed)) {
    uint8_t version;            /* should be = STACK_MAP_VERSION */
    uint8_t reserved1;
    uint16_t reserved2;
...
typedef struct __attribute__((packed)) {
    ...
    uint16_t locSize;   // location size
...
static inline bool isBasePointer (value_location_t *first, value_location_t *second)
{
    return (first->kind == second->kind) && (first->offset == second->offset);
}

static inline bool isIndirect (value_location_t *p)
{
 ...
 */
static inline uint64_t hashFn (uint64_t x)
{
    x ^= x >> 12; // a
        x ^= x << 25; // b
        x ^= x >> 27; // c
...
static inline uint64_t computeBucketIndex (statepoint_table_t* table, uint64_t key)
{
...
static inline size_t size_of_frame (uint16_t numSlots)
{
    return sizeof(frame_info_t) + numSlots * sizeof(pointer_slot_t);
}

static inline size_t frame_size (frame_info_t* frame)
{
    return size_of_frame (frame->numSlots);
}

// returns the next frame relative the current frame
static inline frame_info_t *next_frame (frame_info_t *cur)
{
